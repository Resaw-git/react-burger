...
import MobileConstructorElement from "../constructor-item/mobile-constructor-element";
...
const MobileConstructor = () => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.ingredients}>
        {constructorBun.length > 0 && (
          <MobileConstructorElement
            ...
            image={constructorBun[0].image_mobile}
        ...
        {constructorIng.map((el, index) => {
          return (
            <MobileConstructorElement id={el.id} text={el.name} image={el.image_mobile} price={el.price} key={index} />
          );
        })}
        {constructorBun.length > 0 && (
          <MobileConstructorElement
      ...
      {openOrder && (
        <MobileModal onClose={closeModal} title="Заказ оформлен">
          <MobileOrderDetails />
        </MobileModal>
      )}
    </div>
